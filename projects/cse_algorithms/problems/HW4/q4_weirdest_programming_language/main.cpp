#include <iostream>
#include <vector>
#include <algorithm>
#include <cstring>

constexpr int MOD = 1000000009;
constexpr int MAX_MONSTERS = 1000;
constexpr int MAX_SIZE = 1 << 16;  // Reduced from 2^20 to 2^16

// Fast modulo multiplication to avoid overflow
inline long long mul_mod(long long a, long long b) {
    return (a * b) % MOD;
}

// Fast polynomial multiplication using Karatsuba algorithm
void karatsuba_multiply(const long long* a, const long long* b, long long* result, int n) {
    if (n <= 16) {  // Reduced threshold for naive multiplication
        memset(result, 0, (2 * n - 1) * sizeof(long long));
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (i + j < 2 * n - 1) {
                    result[i + j] = (result[i + j] + mul_mod(a[i], b[j])) % MOD;
                }
            }
        }
        return;
    }
    
    int mid = n / 2;
    
    // Allocate temporary arrays on stack with reduced size
    long long a0[MAX_SIZE/4], a1[MAX_SIZE/4];
    long long b0[MAX_SIZE/4], b1[MAX_SIZE/4];
    long long z0[MAX_SIZE/2], z1[MAX_SIZE/2], z2[MAX_SIZE/2];
    
    // Split polynomials
    memcpy(a0, a, mid * sizeof(long long));
    memcpy(a1, a + mid, (n - mid) * sizeof(long long));
    memcpy(b0, b, mid * sizeof(long long));
    memcpy(b1, b + mid, (n - mid) * sizeof(long long));
    
    // Recursive calls
    karatsuba_multiply(a0, b0, z0, mid);
    karatsuba_multiply(a1, b1, z2, n - mid);
    
    // Compute (a0 + a1)(b0 + b1)
    for (int i = 0; i < mid; i++) {
        a0[i] = (a0[i] + a1[i]) % MOD;
        b0[i] = (b0[i] + b1[i]) % MOD;
    }
    karatsuba_multiply(a0, b0, z1, mid);
    
    // Combine results
    memset(result, 0, (2 * n - 1) * sizeof(long long));
    for (int i = 0; i < 2 * mid - 1; i++) {
        result[i] = z0[i];
    }
    for (int i = 0; i < 2 * (n - mid) - 1; i++) {
        result[i + 2 * mid] = z2[i];
    }
    for (int i = 0; i < 2 * mid - 1; i++) {
        result[i + mid] = (result[i + mid] + z1[i] - z0[i] - z2[i] + 3 * MOD) % MOD;
    }
}

// Fast polynomial exponentiation
void poly_pow(const long long* poly, long long* result, int n, int power) {
    if (power == 0) {
        memset(result, 0, n * sizeof(long long));
        result[0] = 1;
        return;
    }
    if (power == 1) {
        memcpy(result, poly, n * sizeof(long long));
        return;
    }
    
    // Allocate temporary array for intermediate results
    long long temp[MAX_SIZE/2];
    
    // Binary lifting
    poly_pow(poly, temp, n, power / 2);
    karatsuba_multiply(temp, temp, result, n);
    
    if (power % 2) {
        memcpy(temp, result, n * sizeof(long long));
        karatsuba_multiply(temp, poly, result, n);
    }
}

long long solve(int g, long long n_total, const std::vector<int>& monsters) {
    if (monsters.empty()) return 0;
    
    // Find maximum monster value
    int max_monster = *std::max_element(monsters.begin(), monsters.end());
    if (max_monster == 0) return 0;
    
    // Create polynomial coefficients with reduced size
    long long coeffs[MAX_SIZE/2] = {0};
    for (int monster : monsters) {
        if (monster > 0 && monster < MAX_SIZE/2) {
            coeffs[monster] = 1;
        }
    }
    
    // Calculate result
    long long result[MAX_SIZE/2];
    poly_pow(coeffs, result, max_monster + 1, n_total);
    
    return n_total < MAX_SIZE/2 ? result[n_total] : 0;
}

int main() {
    std::ios_base::sync_with_stdio(false);  // Optimize I/O
    std::cin.tie(nullptr);
    
    int t;
    std::cin >> t;
    
    while (t--) {
        int g;
        long long n_total;
        std::cin >> g >> n_total;
        
        if (g <= 0 || n_total <= 0) {
            std::cout << "0\n";
            continue;
        }
        
        std::vector<int> monsters(g);
        for (int i = 0; i < g; i++) {
            std::cin >> monsters[i];
        }
        
        std::cout << solve(g, n_total, monsters) << '\n';
    }
    
    return 0;
} 